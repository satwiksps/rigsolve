...
def test_lockfile_write_failure_preserves_existing_destination(tmp_path, monkeypatch) -> None:
    from rigsolve.plan.lockfile import write_lockfile

    path = tmp_path / "rigsolve.toml"
    original = b"existing lockfile\n"
    ...
    with pytest.raises(OSError, match="simulated replace failure"):
        write_lockfile(sample_plan(), path)
...
def test_docker_and_colab_outputs_are_complete() -> None:
    ...
    assert docker == "\n".join(
        (
            ...
            "# Generated from rigsolve matrix 2026.08.15; evidence: metadata-backed.",
            ...
            "RUN uv pip install --system --python /usr/local/bin/python "
            "'https://example.test/flash.whl#sha256=" + "b" * 64 + "'",
            "",
            'CMD ["python"]',
            "",
        )
    ...
    assert colab == "\n".join(
        (
            ...
            "# Matrix 2026.08.15 (aaaaaaaaaaaa); evidence: metadata-backed.",
            "python -m pip install --index-url "
            "https://download.pytorch.org/whl/cu124 torch==2.6.0+cu124",
            "python -m pip install 'https://example.test/flash.whl#sha256=" + "b" * 64 + "'",
            "",
        )
    )
